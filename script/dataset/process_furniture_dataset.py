...
def make_dataset(load_dir, save_dir, save_name_prefix, val_split=0.0, normalize=False):
    ...
    for pkl_file in tqdm(pkl_files, desc='Processing .pkl files'):
        ...
        # Extract robot states from observations
        states = [obs['robot_state'] for obs in observations]
    ...
    out_val = {'states': [], 'actions': [], 'traj_lengths': []}
    ...
    if normalize:
        normalization_save_path = os.path.join(save_dir, save_name_prefix + 'normalization.npz')
        np.savez_compressed(
